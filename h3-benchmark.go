...
func logit(msg string) {
	fmt.Println(msg)
	logfile, _ := os.OpenFile("h3-benchmark.log", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
	if logfile != nil {
...
func runUpload(thread_num int) {
	for time.Now().Before(endtime) {
		...
		status := C.h3lib_write_object(handle, bucket_str, objectName, unsafe.Pointer(&object_data[0]), C.ulonglong(object_size), C.ulonglong(0))
		if status != C.H3_SUCCESS{
